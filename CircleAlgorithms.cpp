...
void ChooseCircleAlgo(const Algorithm &currentCircleAlgorithm, Point circlePoints[], COLORREF color , HDC hdc );

LRESULT WINAPI
drawCircle(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, Algorithm currentCircleAlgorithm, COLORREF color,
           DrawCommand &cmd) {
    HDC hdc;
        ...
        case WM_LBUTTONDOWN:
            if (circlePointCount < 2) {
                ...
                if (circlePointCount == 2) {
                    ...
                    ChooseCircleAlgo(currentCircleAlgorithm , circlePoints,color ,hdc );
...
void ChooseCircleAlgo(const Algorithm &currentCircleAlgorithm, Point circlePoints[], COLORREF color , HDC hdc ) {
    ...
    switch (currentCircleAlgorithm) {
        case ALGO_CIRCLE_DIRECT:
            CircleDirect(hdc, circlePoints[0].x, circlePoints[0].y, R, color);
            break;
        case ALGO_CIRCLE_POLAR:
            CirclePolar(hdc, circlePoints[0].x, circlePoints[0].y, R, color);
            break;
        case ALGO_CIRCLE_ITERATIVE_POLAR:
            CircleIterativePolar(hdc, circlePoints[0].x, circlePoints[0].y, R, color);
            break;
        case ALGO_CIRCLE_MIDPOINT:
            CircleBresenham(hdc, circlePoints[0].x, circlePoints[0].y, R, color);
            break;
        case ALGO_CIRCLE_MODIFIED_MIDPOINT:
            CircleModifiedBresenham(hdc, circlePoints[0].x, circlePoints[0].y, R, color);
            break;
    }
}
